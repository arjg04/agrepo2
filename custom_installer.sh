...
if [ $installoption == 'a' ] ; then
    ...
    if [ $deoption == 'a' ] ; then
        ...
        read username
        sudo chroot /mnt /bin/bash -c "apt update ; useradd -m -s /bin/bash $username ; echo Enter the password for $username ; passwd $username ; usermod -aG audio,video,sudo $username ; echo $computername > /etc/hostname ; apt install -y linux-image-amd64 firmware-linux efibootmgr os-prober dosfstools mtools vim sudo nano network-manager gnome firefox-esr grub-efi-amd64 plymouth-themes wget curl command-not-found net-tools vlc ; grub-install --target=x86_64-efi --bootloader-id=debian --efi-directory=/boot/efi --recheck --force ; sed 's/GRUB_CMDLINE_LINUX_DEFAULT=\"quiet\"/GRUB_CMDLINE_LINUX_DEFAULT=\"quiet splash vt.global_cursor_default=0\"/g' > /etc/default/grub ; plymouth-set-default-theme -R bgrt ; update-grub ; systemctl enable NetworkManager ; systemctl enable gdm"
        ...
        echo 'Enter the new name of the user:'
        read username
         sudo chroot /mnt /bin/bash -c "apt update ; useradd -m -s /bin/bash $username ; echo Enter the password for $username ; passwd $username ; usermod -aG audio,video,sudo $username ; echo $computername > /etc/hostname ; apt install -y linux-image-amd64 firmware-linux efibootmgr os-prober dosfstools mtools vim sudo nano network-manager kde-standard konsole kate kwrite kio-admin firefox-esr grub-efi-amd64 plymouth-themes wget curl command-not-found net-tools vlc ; grub-install --target=x86_64-efi --bootloader-id=debian --efi-directory=/boot/efi --recheck --force ; sed 's/GRUB_CMDLINE_LINUX_DEFAULT=\"quiet\"/GRUB_CMDLINE_LINUX_DEFAULT=\"quiet splash vt.global_cursor_default=0\"/g' > /etc/default/grub ; plymouth-set-default-theme -R bgrt ; update-grub ; systemctl enable NetworkManager ; systemctl enable sddm"
    else
    ...
    if [ $deoption == 'a' ] ; then
        ...
        sudo chroot /mnt /bin/bash -c "apt update ; useradd -m -s /bin/bash $username ; echo Enter the password for $username ; passwd $username ; usermod -aG audio,video,sudo $username ; echo $computername > /etc/hostname ; apt install -y linux-image-amd64 firmware-linux efibootmgr os-prober dosfstools mtools vim sudo nano network-manager gnome-shell gdm3 $libreoffice $firefoxesr $gnomegames $vlc nautilus gnome-calculator grub-efi-amd64 plymouth-themes wget curl command-not-found net-tools vlc ; grub-install --target=x86_64-efi --bootloader-id=debian --efi-directory=/boot/efi --recheck --force ; sed 's/GRUB_CMDLINE_LINUX_DEFAULT=\"quiet\"/GRUB_CMDLINE_LINUX_DEFAULT=\"quiet splash vt.global_cursor_default=0\"/g' > /etc/default/grub ; plymouth-set-default-theme -R bgrt ; update-grub ; systemctl enable NetworkManager ; systemctl enable gdm"
        ...
        sudo chroot /mnt /bin/bash -c "apt update ; echo Enter the password for $username ; passwd $username ; usermod -aG audio,video,sudo $username ; echo $computername > /etc/hostname ; apt install -y linux-image-amd64 firmware-linux efibootmgr os-prober dosfstools mtools vim sudo nano network-manager kde-plasma-desktop kcalc konsole kate $libreoffice $firefoxesr $kdegames $vlc grub-efi-amd64 plymouth-themes wget curl command-not-found net-tools vlc ; grub-install --target=x86_64-efi --bootloader-id=debian --efi-directory=/boot/efi --recheck --force ; sed 's/GRUB_CMDLINE_LINUX_DEFAULT=\"quiet\"/GRUB_CMDLINE_LINUX_DEFAULT=\"quiet splash vt.global_cursor_default=0\"/g' > /etc/default/grub ; plymouth-set-default-theme -R bgrt ; update-grub ; systemctl enable NetworkManager ; systemctl enable gdm"
    else
        echo 'That is not a valid option. Setup will now exit'
        exit
    fi
